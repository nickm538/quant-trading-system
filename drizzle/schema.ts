...
export const optionsAnalysisScan = mysqlTable("options_analysis_scan", {
  ...
  // Top recommendations
  topCallScore: int("top_call_score"),
  topPutScore: int("top_put_score"),
  
  // Market conditions
  stockPrice: int("stock_price").notNull(),
  marketVolatility: int("market_volatility"),
  
  // Scan metadata
  scanTimestamp: timestamp("scan_timestamp").notNull(),
  scanDurationMs: int("scan_duration_ms").notNull(),
  engineVersion: varchar("engine_version", { length: 20 }).notNull(),
